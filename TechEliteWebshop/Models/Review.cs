...
namespace TechEliteWebshop.Models
{
    ...
    {
        [Key]
        public string ReviewId { get; set; } = GenerateReviewId();

        [ForeignKey("Product")]
        public required string ProductId { get; set; } // Nullable för att kunna skapa recensioner utan att ha en produkt kopplad
        public required Product Product { get; set; } // Navigeringsegenskap
        ...
        {
            return $"REV-{Guid.NewGuid().ToString().Substring(0, 8).ToUpper()}";
        }
    }
}
